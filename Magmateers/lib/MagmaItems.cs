...
using static Magmateers.lib.data.ExportHandler;
using Magmateers.lib.data;
using System.Collections.Generic;
namespace Magmateers.lib
{
    class MagmaItems
    {
        public MagmaItems()
        {
            foreach (KeyValuePair<Item, GUID> dict in magmaItems)
            {
            ...
            MagmaLog.Log("[Magmateers | Items]: Items Loaded...");
        }
        ...
        private Sprite Sprite2(string iconpath)
        {
            ...
            if (!File.Exists(path))
            {
                MagmaLog.Log("ERROR: [Magmateers | Items]: Specified Icon path not found: " + path);

                Debug.LogError("[Magmateers | Items]: Specified Icon path not found: " + path);
                return null;
            }
        ...
        private void CreateItem(string codename, int maxstack, LocalizedString name, LocalizedString desc, string guidstring, string recipecategoryname, Sprite icon)
        {
            ...
            MagmaDict.magmaRegistry[codename] = guid;
            MagmaLog.Log("[Magmateers | Items]: Item " + codename + " has been loaded");
        }
    }
}
